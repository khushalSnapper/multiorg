...
const blockListener = async (event) => {
    ...
    console.log("transEvents length ", transEvents.length)
    for (const transEvent of transEvents) {
        console.log(`*** transaction event: ${transEvent.transactionId}`);
        if (transEvent.privateData) {
            for (const namespace of transEvent.privateData.ns_pvt_rwset) {
                ...
                for (const collection of namespace.collection_pvt_rwset) {
                    ...
                    if (collection.rwset.writes) {
                        console.log("collection.rwset.writes", collection.rwset.writes.length)
                        for (const write of collection.rwset.writes) {
                            ...
                            console.log(typeof event.blockNumber.low)
                            //    var temp = event.blockNumber.low;
                            ...
                            if (validEvent) {
                                ...
                                if (privateData.orgName != constants.orgName) {
                                    let returnResult = request.post({
                                        url: 'http://20.221.240.130:3000/api/v1/blockchainEvents/blockchain_events',
                                        json: true,
                                        traditional: true,
                                        body: b
                                    }, (err, result) => {
                            ...
                            // console.log(returnResult.Request.body);
                            // finalResult=returnResult.Request.new;
                            // if(JSON.stringify(b) === JSON.stringify(finalResult)){
                            //     console.log("******************************",returnResult.Request.new )
                            // } else {
                            //     console.log("#####################")
                            // }
...
// const checkBlockNumber = async function (blockNumber) {
//     console.log("temp blockNumber", temp, blockNumber)
//     if (temp !== blockNumber) {
//         return true;
